...
class AssemblyClient(APIClient):
    ...
    def create_task_definition(
        ...
        params = self._map_parameters(
            locals(), self.create_task_definition.actual_kwargs, TASK_DEFINITION_ARG_MAP
        )
    ...
    def get_task(
        self, task_id, include_detail=False, include_assignments=False
    ) -> Task:
        if include_detail:
            return Task(
                self.get(
    ...
    def list_workers(self, definition_id):
        ...
        while start_key:
            ...
            response = self.get(
                f"{self.ENDPOINT}/taskDefinition/{definition_id}/workers", params
            )
    ...
    def list_tasks(
        self, task_definition_id=None, tag=None, tag_value=None, batch_id=None
    ):
