...
class CLI:
    ...
    def create_task_definition(self, name, task_type_id):
        ...
        with open("definition.yaml", "w") as fp:
            yaml.dump(definition, fp)
        print(
            f"Created task definition {definition['DefinitionId']} in definition.yaml"
        )
    ...
    def get_batch_status(self, batch_id):
        ...
        print(
            f"     Total: {response.get('AssignmentCount', 0) + response.get('TestResponseCount', 0)}"
        )
        ...
        if spend and (spend.get("TaskRewardCents") or spend.get("TestRewardCents")):
            ...
            if spend.get("TaskRewardCents"):
                print(
                    f"     Task Reward: ${z.format(spend.get('TaskRewardCents', 0) / 100)}"
                )
                print(
                    f"     Task Fees: ${z.format(spend.get('TaskFeeCents', 0) / 100)}"
                )
            if spend.get("TestRewardCents"):
                print(
                    f"     Test Reward: ${z.format(spend.get('TestRewardCents', 0) / 100)}"
                )
                print(
                    f"     Test Fees: ${z.format(spend.get('TestFeeCents', 0) / 100)}"
                )
                ...
                if ext == "jsonl":
                    ...
                    with open(output_file, "w", newline="") as fp:
                        writer = csv.DictWriter(
                            fp, fieldnames=list(fieldnames.keys()), delimiter=delimiter
                        )
                        writer.writeheader()
                        for result in results:
                    ...
                    raise Exception(
                        "Input file must have an extension of json, jsonl, csv, tsv, or txt"
                    )
    ...
    def list_workers(self, definition_file, output_file):
        ...
        with open(output_file, "w", newline="") as fp:
            writer = csv.DictWriter(
                fp,
                fieldnames=["WorkerId", "Submitted", "ScoredCount", "Points", "Score"],
            )
            writer.writeheader()
            for worker in workers:
                if (
                    worker.get("ScoredCount", 0) > 0
                    and worker.get("Points") is not None
                ):
    ...
    def list_tasks(
        ...
        if os.path.exists(definition_file):
            def_id = (
                ...
                else self.read_definition(definition_file)["DefinitionId"]
            )
        if tag:
            tasks = self.client.list_tasks(
                tag=tag[0], tag_value=tag[1], task_definition_id=def_id
            )
        elif batch_id:
    ...
    def close_testing(self, definition_file, min_score=None):
        ...
        qualification_id = lry.mturk.create_qualification_type(
            f"Good {definition['DefinitionId']}", f"Good {definition['DefinitionId']}"
        )
        ...
        if failures:
            print(
                f"Failed to add the following {len(failures)} workers to the qualification:"
            )
            for w in failures:
                print(w)
    ...
    def list_batches(self, definition_file, output_file=None, all_definitions=False):
        ...
        for batch in batches:
            batch["Created"] = datetime.fromisoformat(batch["Created"]).strftime(
                "%m/%d/%Y %H:%M"
            )
...
def main():
    ...
    if args.command == "configure" and args.validate:
        ...
        print(
            f"MTurk connection status: {'SUCCESS' if response.get('MTurk') else 'FAILED'}"
        )
        print(
            f"MTurk Sandbox connection status: {'SUCCESS' if response.get('MTurkSandbox') else 'FAILED'}"
        )
    elif args.func:
